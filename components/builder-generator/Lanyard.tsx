...
const LANYARD_PNG_PATH = "/assets/lanyard/lanyard.png";
const ID_BACK_PNG_PATH = "/assets/lanyard/ID_back.png";

import * as THREE from "three";
import "./Lanyard.css";

// Suppress deprecation warnings from internal @react-three/fiber & three-stdlib calls in Three.js r185+
...
// Preload 3D assets & textures to ensure zero load delays or failures
try {
  useGLTF.preload(CARD_GLB_PATH);
  useTexture.preload(LANYARD_PNG_PATH);
  useTexture.preload(ID_BACK_PNG_PATH);
  useTexture.preload(BLANK_PIXEL);
} catch {
  // Preload fallback if SSR or window undefined
}
...
function Band({
  ...
  const glbSrc = CARD_GLB_PATH;
  const lanyardSrc = lanyardImage || LANYARD_PNG_PATH;
